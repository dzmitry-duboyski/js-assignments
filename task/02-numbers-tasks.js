...
//return Math.acos((x1 * x2 + y1 * y2) / (Math.sqrt((x1, 2) + Math.pow(y1, 2)) * Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2))));
...
function toNumber(value, def) {
  return isNaN(value) > false ? (Number(def)) : (Number(value) + Number(def));
}
